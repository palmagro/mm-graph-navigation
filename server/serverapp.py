...
def peticion_n():
	...
	# consulta del primer nodo por propiedad y valor de propiedad
		...
		consulta=json.dumps({"query" : "MATCH (n:"+c["dato"] +") where n."+c["etiqueta"]+" ='"+str(c["id"])+"' RETURN n, ID(n) ","params" : {	  }})
		#consulta=json.dumps({"query" : "MATCH (n:"+c["dato"] +") where n.Descripcion ='"+str(c["id"])+"' RETURN n, ID(n) ","params" : {	  }})
		print consulta
